#!/usr/bin/env bash
# ============================================================
# Agentic Multi Cloud Security Assessment Runner - v4.2.2
# Author: Wagner Azevedo
# Created on: 2025-10-22T00:29:00Z
# Changes in this revision:
# - FIX: Corrected expansion syntax for Prowler output flags (resolved '---output-formats' error).
# - HARDEN: Safe logging and execution structure maintained.
# ============================================================
...
authenticate() {
    ...
    aws)
        ...
        # CORREÇÃO: Usar expansão simples para evitar o erro '---output-formats'
        if ! prowler aws \
            ${OUTPUT_FLAG:-} ${OUTPUT_VALUE:-} \
            ...
            if [[ "$OUTPUT_FLAG" == "-M" ]]; then
                # Tentativa com --output-formats (legado)
...
cd /
# Garantindo que o caminho de origem para o 'aws s3 cp' seja o diretório de saída
